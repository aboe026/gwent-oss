...
| codegen does not produce `.mts` files                                                                                                                                                                                              | Rename generated `.ts` to `.mts`                                                                                                                                                                                            |                                                                  |
| ESM errors with Error: Qualified path resolution failed: we looked for the following paths, but none could be accessed. Source path: gwent\.yarn\cache\graphql-npm-16.9.0-a36f71845f-5833f82bb6.zip\node_modules\graphql\utilities | use node-modules nodeLinker in [.yarnrc.yml](./.yarnrc.yml)                                                                                                                                                                 |                                                                  |
| ESM errors with `SyntaxError: The requested module 'log4js' does not provide an export named 'getLogger'` and `SyntaxError: The requested module 'body-parser' does not provide an export named 'json'`                            | Change `import { getLogger } from 'log4js'` to `import log4js from 'log4js` and do `log4js.getLogger` instead of just `getLogger`                                                                                           |                                                                  |
