...
test('Horn effects multiple units with different names', async (t) => {
  const unitName1 = 'Blueboy Lugos'
...
test('Horn effects multiple units with same names', async (t) => {
  ...
  const deckUnit1 = await gameManager.deploy({ unitName: unitName1 })
  await gameManager.pass({})
  const deckUnit2 = await gameManager.deploy({ unitName: unitName1 })
  await gameManager.initialize({})
