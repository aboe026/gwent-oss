...
export default function env() {
  return getEnv({
    dotEnvFilePath: process.env.NODE_ENV === NODE_ENV.Dev ? path.join(import.meta.dirname, '..', '.env') : '',
    specs: {
      ...
      LOG_LEVEL: str({
        desc: 'The minimum granularity level of log messages which should be output. OFF < FATAL < ERROR < WARN < INFO < DEBUG < TRACE < ALL.',
